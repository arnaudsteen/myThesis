...
\makeatother
\title{\Huge Caractérisation des gerbes hadroniques à l'aide d'un calorimètre à lecture semi-digital}
...
\jury{
  \begin{center}
    \begin{tabular}{lll}
      JURY~: & M.  Jean-Claude \textsc{Brient} & (rapporteur)\\
      & M.  Paul \textsc{Colas} & (rapporteur)\\
      & M.  Didier \textsc{Contardo} & (examinateur)\\
      & M.  Gérald \textsc{Grenier} & (co-directeur de th\`ese)\\
      & Pr.  Imad  \textsc{Laktineh} & (directeur de th\`ese)\\
      & M.  Felix \textsc{Sefkow} & (examinateur)\\
    \end{tabular}
  \end{center}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}
  \begin{center}
    ...
    \hspace{1cm}
    \includegraphics[width=0.18\textwidth]{Logo/logo_ipnl.png}
    \hspace{1cm}
